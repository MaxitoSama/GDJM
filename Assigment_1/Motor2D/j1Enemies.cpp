...
bool j1Enemies::Update()
{
	...
	else
	{
		...
		for (uint i = 0; i < MAX_ENEMIES; ++i)
			if (enemies[i] != nullptr && (enemies[i]->collider != nullptr) && (enemies[i]->collider->type == COLLIDER_NONE/*COLLIDER_DECORATION*/) && (enemies[i]->lower_level == false)) enemies[i]->Draw(enemies[i]->sprites);

		for (uint i = 0; i < MAX_ENEMIES; ++i)
			if (enemies[i] != nullptr && (enemies[i]->collider != nullptr) && (enemies[i]->collider->type == COLLIDER_NONE/*COLLIDER_ENEMY*/) && (enemies[i]->lower_level == false)) enemies[i]->Draw(enemies[i]->sprites);
		...
		for (uint i = 0; i < MAX_ENEMIES; ++i)
			if (enemies[i] != nullptr && (enemies[i]->collider != nullptr) && (enemies[i]->collider->type == COLLIDER_NONE/*COLLIDER_AIR_ENEMY*/) && (enemies[i]->lower_level == false)) enemies[i]->Draw(enemies[i]->sprites);
	}
...
bool j1Enemies::PostUpdate()
{
	...
	for(uint i = 0; i < MAX_ENEMIES; ++i)
	{
		if(enemies[i] != nullptr)
		{
			if (enemies[i]->position.x < -(SPAWN_MARGIN + 1) || enemies[i]->position.x >(App->render->camera.w + (SPAWN_MARGIN + 1)) || enemies[i]->position.y > (-App->render->camera.y + App->render->camera.h + (SPAWN_MARGIN + 1)) || enemies[i]->position.y < (-App->render->camera.y - (SPAWN_MARGIN + 1)))
